 ...
 AppConfigを使って、データ分析エージェントを作るリポジトリです。
...
3つのStepを踏んで、AWS AppConfigを使ったアプリケーションの設定管理とデータ分析エージェントの作成します。
...
依存関係を入れた仮想環境を使って、以下のコマンドでテストを実行できます。
...
または、既存の Make タスクを使う場合は次のように実行できます。
...
### 5. ローカルで OpenAPI / Swagger UI を確認する

ローカルでも FastAPI の仕様書を確認できます。まず依存関係を入れた仮想環境を有効化し、以下のコマンドでサーバーを起動します。

```bash
source .venv/bin/activate
PYTHONPATH=. uvicorn app.step4_bedrock:app --reload --host 127.0.0.1 --port 8000
```

その後、ブラウザで次の URL を開くと Swagger UI から API を確認できます。
